  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        console.log("✅ Recent Scores (from /games/matches/history/):", historyRes.data);
    ...
    datasets: [
      {
        ...
        backgroundColor: last5.map((m) =>
          m.winner === user.full_name ? 'rgba(34,197,94,0.7)' : 'rgba(239,68,68,0.7)'
        ),
        borderColor: last5.map((m) =>
          m.winner === user.full_name ? 'rgba(34,197,94,1)' : 'rgba(239,68,68,1)'
        ),
        borderWidth: 1,
      },
      {
        type: 'line',
        label: 'Avg Score',
        data: Array(last5.length).fill(
          last5.reduce((sum, m) => sum + m.player_score, 0) / last5.length
        ),
