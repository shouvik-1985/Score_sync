...
class SubmitPointView(APIView):
    ...
    def post(self, request, match_id):
        ...
        # Save point updates immediately
        match.save()
        ...
        if game_winner:
            ...
            if match.player1_score == 2:
                match.status = "completed"
                match.winner = match.challenge.sender
                match.ended_at = timezone.now()
                if match.challenge:
                ...
                match.status = "completed"
                match.winner = match.challenge.receiver
                match.ended_at = timezone.now()
                if match.challenge:
                    match.challenge.status = "completed"
                    match.challenge.save()
            else:
                # Only create a new set if match not over
                if set_winner:
...
class CompleteMatchView(APIView):
    ...
    def post(self, request, match_id):
        ...
        return Response({"message": "Match completed", "winner": match.winner.username})
